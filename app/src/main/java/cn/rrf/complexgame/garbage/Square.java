...
public class Square {
    ...
    public Square(int identifier, float[] projection, float[] view, Vec translate, Vec scale) {
        float[] vertices = {
                0.5f, 0.5f, 0.0f,
                -0.5f, 0.5f, 0.0f,
                -0.5f, -0.5f, 0.0f,
        ...
        verticesBuffer = ByteBuffer.allocateDirect(vertices.length * Float.SIZE / 8).order(ByteOrder.nativeOrder()).asFloatBuffer().put(vertices);
        verticesBuffer.position(0);
        indicesBuffer = ByteBuffer.allocateDirect(indices.length * Short.SIZE / 8).order(ByteOrder.nativeOrder()).asShortBuffer().put(indices);
